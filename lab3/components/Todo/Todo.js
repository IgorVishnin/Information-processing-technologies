...
function TodoList(props) {
    ...
    if (list !== undefined) {
        ...
        return ( <
            ...
            onPressEnter = {
                (e) => handlerAddToTodo(e) }
            /> {
                pageList.map((item, index) => {
                    return ( <
                        List.Item key = { index }
                        style = {
                            { listStyle: 'decimal', textDecoration: item.completed ? 'line-through' : 'none', display: 'flex' } } >
                        <
                        div >
                        <
                        Checkbox onChange = {
                            () => handelerTodoChecked(item.id) }
                        ...
                        onClick = {
                            () => handlerDeleteTodo(item.id) } > Удалить < /Button> <
                        /List.Item>
                    )
                })
            } <
            ...
            showSizeChanger showQuickJumper defaultCurrent = { currentPage }
